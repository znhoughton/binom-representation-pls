...
\item
  ...
  the probe can only use what each word carries on its own. Anything
  that requires the two words together is removed.
\item
...
\texttt{log\_freq} is centred but not scaled, so both mean the same
...
Because \texttt{log\_freq} is centred within each fit, a main effect is
...
\(-0.5\) to \(+0.5\) and \texttt{log\_freq} is centred but not scaled,
