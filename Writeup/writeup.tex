...
words and not only the representation of the phrase. Tracking training
checkpoints further demonstrates that abstract preferences are present
early while the frequency-dependent storage is not observable until
later, the point at which it becomes detectable being governed by
...
\item
  ...
  ordering preferences for binomials it hasn't seen, however the order a
...
\item
  ...
  representations.} Part of what a model knows about a particular
  binomial survives an intervention that removes its phrasal
  representation, indicating that repeated exposure to a phrase also
  changes the representations of the individual words it contains.
\end{itemize}
...
every corpus. Coefficients are therefore comparable across checkpoints
...
it shifts the phrasal one. The probe trajectories are consistent with
this. For the BabyLMs, probe R² falls as the effect of relative
frequency rises, and falls almost in step with it; for Pythia the two
rise together. In other words, for the models that see the same 150M
tokens twenty times, later training seems to add item-specific knowledge
without adding structure that generalizes across items, so a probe
trained to generalize recovers less of the preference. The attention
conditions follow the same course: zeroing out attention \emph{helps} at
the earliest checkpoints and hurts by the end, which is what we would
expect if there is no phrasal representation to remove early on. That
the phrasal representation comes to carry ordering information is not
especially surprising, since attention is what allows a representation
of the pair to be built at all. That the \emph{lexical} representations
shift is the more interesting result, and it parallels evidence from
human processing that the internal representations of a stored
multi-word phrase are themselves altered by experience with the phrase
...
corpus (\texttt{rel\_freq}), total binomial frequency
...
that model's own preference spread. \texttt{rel\_freq} and
\texttt{log\_freq}, by contrast, are properties of the training corpus:
a proportion of occurrences and a count of them mean the same thing
however they are measured, and dividing either by a corpus-specific
standard deviation would replace a shared unit with a local one. They
are therefore left on their own scales: \texttt{rel\_freq} is a
proportion running from \(-0.5\) to \(+0.5\) and \texttt{log\_freq} is
centred but not scaled, so both mean the same thing in every corpus. A
\texttt{rel\_freq} coefficient is therefore the change in ordering
preference, in standard deviations, between a binomial always attested
in one order and one always attested in the other; a \texttt{log\_freq}
coefficient is the change per unit of log frequency, which is a factor
of \(e \approx 2.72\) in the raw count; multiplying by \(0.69\) converts
it to the change per doubling. Because \texttt{log\_freq} is centred
within each fit, a main effect is the effect at that corpus's average
binomial, which is a different absolute frequency for BabyLM than for
the large-scale corpora. Main effects are therefore interpretable within
a fit and comparable between attention conditions, but they are not
comparable in magnitude across the two corpora. The two predictors are
also on different scales, so their coefficients should not be compared
with each other.
...
below 0.5. Rank correlations track the Pearson values closely
(0.371--0.406 for BabyLM on novel binomials), and the models order the
same binomial the same way on 60\% to 70\% of items against a chance
rate of 50\%, a statistic that ignores magnitude and weights every
binomial equally. Stratifying by preference strength, using the third
model's \(|y_{\text{true}}|\) so that the bins are independent of the
two series being correlated, agreement is graded but present throughout:
in the weakest quintile \(r = 0.30\) under full context and \(0.28\)
under attention-zeroing, with directional agreement near 59\% in both,
rising to \(r = 0.61\) and 73\% in the strongest. That gradient is
itself informative, since agreement increasing with preference strength
is the signature of a signal measured with noise rather than of an
arbitrary function of a particular set of weights.
