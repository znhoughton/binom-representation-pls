...
#       python Scripts/supplementary/supplementary_jobs.py | column -t
#
# RESTARTING
...
#
# Usage:
#   bash Scripts/supplementary/supplementary_analyses.sh
#   ONLY="babylm pythia"        bash Scripts/supplementary/supplementary_analyses.sh
#   FINAL_ONLY=1                bash Scripts/supplementary/supplementary_analyses.sh
#   EXCLUDE="allenai_OLMo-2-1124-1B" bash Scripts/supplementary/supplementary_analyses.sh
#   KEEP_EMBEDDINGS=1 SKIP_STEERING=1 bash Scripts/supplementary/supplementary_analyses.sh
#   FORCE=1                     bash Scripts/supplementary/supplementary_analyses.sh
#   LAST_LAYER_ONLY=1           bash Scripts/supplementary/supplementary_analyses.sh
#
...
JOBS="$(mktemp)"
trap 'rm -f "$JOBS"' EXIT
"$PYTHON" "$BASE/Scripts/supplementary/supplementary_jobs.py" "${JOB_ARGS[@]}" > "$JOBS"
N_JOBS=$(wc -l < "$JOBS")
...
while IFS=$'\t' read -r SLUG HF_ID REVISION NLAYERS BATCH LAYERS; do
  ...
  CELL_OK=1
  {
    ...
    CAL_OUT="$("$PYTHON" "$BASE/Scripts/pipeline/extract_embeddings.py" \
    ...
    for COND in "default:binomial:novel_embeddings" \
      ...
      "$PYTHON" "$BASE/Scripts/pipeline/extract_embeddings.py" \
        --model "$HF_ID" "${REV_ARGS[@]}" \
    ...
    "$PYTHON" "$BASE/Scripts/supplementary/supplementary_probes.py" \
    ...
    if [[ "$SKIP_STEERING" != "1" ]] && [[ "$N_L" -ge 3 ]]; then
      ...
      "$PYTHON" "$BASE/Scripts/supplementary/steering.py" \
